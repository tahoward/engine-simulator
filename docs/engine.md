...
- **Combustion uses a [Wiebe function](glossary.md#wiebe-function)**, a standard curve for how fast fuel burns. The total heat
  released depends on how much *fresh* charge is trapped when the intake valve closes. So the
  throttle works the way a real one does, by limiting how much air gets in. Engine load comes out
  of the mass bookkeeping rather than being applied as a volume knob.
