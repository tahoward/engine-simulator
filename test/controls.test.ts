/**
 * `setControls` is the worklet's per-block fast path for throttle and load. It must do exactly what
 * `setEngine` does with the same two values, or the sound would depend on which path a change
 * happened to take.
 */
...
describe('setControls', () => {
  for (const freeRunning of [false, true]) {
    it(`matches setEngine bit for bit (${freeRunning ? 'free-running' : 'fixed rpm'})`, () => {
      ...
      a.setEngine({ throttle: 0.8, load: 0.7 });
      b.setControls(0.8, 0.7);
